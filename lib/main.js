(function() {
    ...
    var parseSubTemplates = function parseSubTemplates(template) {
        ...
        while (template.indexOf("<script") >= 0) {
            ...
            var delim = template.indexOf(">")
            var firstTemplateLine = template.substring(scriptTag, delim) // same here
            templates.push({
                name: firstTemplateLine,
                template: template.substring(scriptTag, closeScriptTag)
            })
    ...
    var createEmberPreprocessor = function(logger, basePath) {
        ...
        return function(content, file, done) {
            ...
            try {
                var template = new Cli({
                    args: [file.originalPath]
